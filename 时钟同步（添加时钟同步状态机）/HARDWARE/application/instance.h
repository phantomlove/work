...
#define TAG_POLL_MSG_LEN                    3				// FunctionCode(1), Range Num (1)£¬Dataheader(1)
...
typedef enum instanceModes{LISTENER, TAG, ANCHOR, ANCHOR_RNG, NUM_MODES,ANCHOR_MASTER} INST_MODE;
...
typedef struct
{
    ...
    INST_STATES previousState ;			//state machine - previous state
		INST_STATES CurrentState_Syc;
		INST_STATES NextState_Syc;
		INST_STATES PreviousState_Syc;
