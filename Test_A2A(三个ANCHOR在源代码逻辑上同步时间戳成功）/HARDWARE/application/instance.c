...
//uint8 TimeSlot_Number_R_TAG = 0;
uint8 TimeSlot_Number_R_Test = 0;
uint8 TimeSlot_Number_A1_FLAG = 0;
uint8 TimeSlot_Number_A2_FLAG = 0;
uint8 TimeSlot_Number_A3_FLAG = 0;
uint8 TimeSlot_Number_TAG_SYC = 1;
uint8 TimeSlot_Number_ANC_SYC = 1;
uint8 TimeSlot_Number_ANC_SYCR = 1;
        ...
        case TA_TXPOLL_WAIT_SEND :
            {
								...
								if(inst->mode != ANCHOR_RNG)
								{
									...
									inst->msg_f.messageData[POLL_RNUM + 1 ] = 0xAA;
									inst->msg_f.messageData[POLL_RNUM + 2 ] = 0xAA;
...
//							dwt_readsystime(&timestamp0);
//							if(inst->msg_f.messageData[POLL_RNUM + 1 ] == 0x5A)
//							{
//								n = 0;
...
//							}
              dwt_writetxdata(inst->psduLength, (uint8 *)  &inst->msg_f, 0) ;	// write the frame data
                ...
                inst->done = INST_DONE_WAIT_FOR_NEXT_EVENT; //will use RX FWTO to time out (set above)
								...
								{
									if(inst->instanceAddress16 == A1_ANCHOR_ADDR)
									{
											n = 0;
											n = sprintf((char*)&LogBuff[0], "T1= %d\r\n",OS_TS_GET());
											USB_TxWrite(LogBuff, n);
										inst->msg_f.messageData[POLL_RNUM] = (inst->mode == TAG) ? inst->rangeNum : inst->rangeNumAnc; //copy new range number
										inst->msg_f.messageData[FCODE] = (inst->mode == TAG) ? RTLS_DEMO_MSG_TAG_POLL : RTLS_DEMO_MSG_ANCH_POLL; //message function code (specifies if message is a poll, response or other...)
										inst->msg_f.messageData[POLL_RNUM + 1 ] = ((uint8)TimeSlot_Number_S);
										inst->msg_f.messageData[POLL_RNUM + 2 ] = ((uint8)(TimeSlot_Number_S>>8));
//										n = 0;
//										n = sprintf((char*)&LogBuff[0], "T= %d\r\n",OS_TS_GET());
//										USB_TxWrite(LogBuff, n);
										inst->psduLength = (TAG_POLL_MSG_LEN + FRAME_CRTL_AND_ADDRESS_S + FRAME_CRC);
										inst->msg_f.sourceAddr[0] = inst->eui64[0]; //copy the address
										inst->msg_f.sourceAddr[1] = inst->eui64[1]; //copy the address
										...
										inst->msg_f.destAddr[1] = 0xff;  //set the destination address (broadcast == 0xffff)
										dwt_writetxdata(inst->psduLength, (uint8 *)  &inst->msg_f, 0) ;	// write the frame data
										//dwt_setrxaftertxdelay(0);  //do not delay
										inst->wait4ack = 0 ;
										dwt_writetxfctrl(inst->psduLength, 0); //write frame control
									...
									else
									{
										inst->msg_f.messageData[POLL_RNUM] = (inst->mode == TAG) ? inst->rangeNum : inst->rangeNumAnc; //copy new range number
										inst->msg_f.messageData[FCODE] = (inst->mode == TAG) ? RTLS_DEMO_MSG_TAG_POLL : RTLS_DEMO_MSG_ANCH_POLL; //message function code (specifies if message is a poll, response or other...)
										inst->msg_f.messageData[POLL_RNUM + 1 ] = ((uint8)TimeSlot_Number_R);
										inst->msg_f.messageData[POLL_RNUM + 2 ] = ((uint8)(TimeSlot_Number_R>>8));
//										n = 0;
        ...
        case TA_TX_WAIT_CONF :
		   //printf("TA_TX_WAIT_CONF %d m%d %d states %08x %08x\n", inst->previousState, message, inst->newReportSent, dwt_read32bitreg(0x19), dwt_read32bitreg(0x0f)) ;
				...
				if(dw_event->type != DWT_SIG_TX_DONE) //wait for TX done confirmation
                {
                    ...
                    inst->done = INST_DONE_WAIT_FOR_NEXT_EVENT;
//										n = 0;
//										dwt_readsystime(&timestamp1);
//										n = sprintf((char*)&LogBuff[0], "T1\r\n");
//										n += sprintf((char*)&LogBuff[n], "%d\r\n",timestamp1);
//										USB_TxWrite(LogBuff, n);
                     break;
					...
					else
                    {
                        ...
                        switch(fcode)
                        {
                            ...
                            case RTLS_DEMO_MSG_TAG_POLL:
                            {
								...
								{
									int16 i;
									uint16 RMSG = messageData[2] + (messageData[3] << 8);
//								n = 0;
//								n = sprintf((char*)&LogBuff[0], "RMSG= %d\r\n",RMSG);
//								USB_TxWrite(LogBuff, n);			
								
									
									if((RMSG == 0) && (RMSG != TimeSlot_Number_R) && ((srcAddr[0] | ((uint32)(srcAddr[1] << 8))) == A1_ANCHOR_ADDR))
									{
										for(i=0;i<6500;i++)
																	{
																		;
...
//									USB_TxWrite(LogBuff, n);
                                ...
                                else //the DW1000 is idle (re-enable from the application level)
                                {
                                	...
                                	inst->testAppState = TA_RXE_WAIT ;              // wait for next frame
														...
														if((TimeSlot_Number_TAG_SYC) && (inst->mode == TAG))
														{
															inst->testAppState = TA_RXE_WAIT ;
															message = 0;
															dwt_setrxtimeout(0);
