...
#define SWS1_TXSPECT_MODE 0x38  //Continuous TX spectrum mode
                             //"1234567812345678"
#define SOFTWARE_VER_STRING    "nVer.  2.10  TREK" //16 bytes!
...
void addressconfigure(uint8 s1switch, uint8 mode)
{
    ...
    if(mode == ANCHOR)
    {
    	if(instance_anchaddr > 4)//before is 3
		{
...
uint32 inittestapplication(uint8 s1switch)
{
    ...
    if((instance_mode == ANCHOR) && (instance_anchaddr > 0x4)) //before is 3
    {
...
//	printf("|*******************(C) COPYRIGHT 2018 研创物联*******************\r\n");
//	printf("|                                                                 \r\n");
//	printf("|            UWB Mini3sPlus - RTLS (Version 20180431)             \r\n");
//	printf("|            YanChuang IOT ltd                                    \r\n");	
//	printf("|                                                                 \r\n");	
//	printf("|*******************(C) COPYRIGHT 2018 研创物联*******************\r\n");			
//	printf("Mini3sPlus 开机次数：%d\r\n"    ,sys_para.start_count);

//	n = sprintf((char*)&SendBuff[0], "|*******************(C) COPYRIGHT 2018 研创物联*******************\r\n");
//	n+=	sprintf((char*)&SendBuff[n], "|                                                                 \r\n");
//	n+=	sprintf((char*)&SendBuff[n], "|            UWB Mini3sPlus - RTLS (Version 20180431)             \r\n");
//	n+=	sprintf((char*)&SendBuff[n], "|            YanChuang IOT ltd                                    \r\n");
//	n+=	sprintf((char*)&SendBuff[n], "|                                                                 \r\n");
//	n+=	sprintf((char*)&SendBuff[n], "|*******************(C) COPYRIGHT 2018 研创物联*******************\r\n");	
//	USB_TxWrite(SendBuff, n);
...
//		printf("等待接收AT命令进行虚拟拨码开关设置\r\n");
    ...
    port_EnableEXT_IRQ(); //enable ScenSor IRQ before starting
		MY_INIT_TIM3(7199,9);//启用定时器3，1000HZ更新速度
...
//		
 	while(1)
	{
    	...
    	int txdiff = (portGetTickCnt() - instance_data[0].timeofTx);
			int instance = 0 ;
//		if(TimeSlot_Number_R_Test)
//		{
//			TimeSlot_Number_R_Test = 0;
//			n = 0;
//			n = sprintf((char*)&LogBuff[0], "TimeSlot_Number_R= %d\r\n",TimeSlot_Number_R);
...
//					USB_TxWrite(LogBuff, n);
			...
			if((TimeSlot_Number_S == 0) || (TimeSlot_Number_S == 5))
				{
					TimeSlot_Number_A1_FLAG = 1;
				}
		
			if(TimeSlot_Number_R == 1000)
				{
					TimeSlot_Number_R = 0;
				}
				
			if(TimeSlot_Number_R == 3)
				{
					TimeSlot_Number_A2_FLAG = 1;
				}
				
			if(TimeSlot_Number_R == 5)
				{
					TimeSlot_Number_A3_FLAG = 0;
				}
			if(TimeSlot_Number_R <= 1)
				{
					TimeSlot_Number_TAG_SYC = 1;
				}
				
			if(TimeSlot_Number_S > 15)
				{
					TimeSlot_Number_ANC_SYC = 0;
				}
				
			if(TimeSlot_Number_R > 15)
				{
					TimeSlot_Number_ANC_SYCR = 0;
				}
...
//			if(TimeSlot_Number_R == 60)
//			{
//				TimeSlot_Number_R_Test = 1;
//			}
				if((TimeSlot_Number_A1_FLAG) && (instance_data[instance].instanceAddress16 == A1_ANCHOR_ADDR))
		{
						TimeSlot_Number_A1_FLAG = 0;
...
//					instance_data[instance].instanceTimerEn = 0;
		...
		if((TimeSlot_Number_A2_FLAG) && (instance_data[instance].instanceAddress16 == A2_ANCHOR_ADDR))
		{
					TimeSlot_Number_A2_FLAG = 0;
						port_DisableEXT_IRQ(); //enable ScenSor IRQ before starting
					//anchor0 sends poll to anchor1
					instance_data[instance].mode = ANCHOR_RNG; //change to ranging initiator
					dwt_forcetrxoff(); //disable DW1000
...
//					instance_data[instance].instanceTimerEn = 0;
		...
		if((TimeSlot_Number_A3_FLAG) && (instance_data[instance].instanceAddress16 == A3_ANCHOR_ADDR))
		{
					TimeSlot_Number_A3_FLAG = 0;
					port_DisableEXT_IRQ(); //enable ScenSor IRQ before starting
					//anchor0 sends poll to anchor1
...
//		        }
            ...
            if(rx == TOF_REPORT_T2A)
            {
            	...
            	n = sprintf((char*)&SendBuff[0], "mc %02x %08x %08x %08x %08x %08x %04x %02x %08x %c%d:%d\r\n",
														valid, instancegetidist_mm(0), instancegetidist_mm(1),
														instancegetidist_mm(2), instancegetidist_mm(3),instancegetidist_mm(4),
														l, r, rangeTime,
...
//														(instance_mode == TAG)?'t':'a', taddr, aaddr);
            	...
            	if(sendTWRRawReports == 1)
            	{
            		n += sprintf((char*)&SendBuff[n], "mr %02x %08x %08x %08x %08x %04x %02x %04x%04x %c%d:%d\r\n",
														...
														l, r, txa, rxa,
														(instance_mode == TAG)?'t':'a', taddr, aaddr);
							 memcpy((u8*)DMA_USART_Buff,(u8*)SendBuff,130);
...
//							memset(SendBuff,0,130);	
              ...
              memcpy((u8*)DMA_USART_Buff,(u8*)SendBuff,65);
							memcpy((u8*)DMA_USART_Buff+65,(u8*)SendBuff,65);
            }
